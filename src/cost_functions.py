...
def FST_norm_1(xs, n_samples, cut):
    ...
    norm = len_in_cut*len_out_cut
    FST_exp = (np.sum(F_in) + np.sum(F_out))*norm
    return 1/FST_exp

def FST_normalized(xs, n_samples, cut):
    len_in_cut = np.count_nonzero(cut == True)
    len_out_cut = np.count_nonzero(cut == False)
    len_xs = len_in_cut + len_out_cut
    p_in = (1 / (2 * len_in_cut)) * np.sum(xs * cut[:, np.newaxis], axis=0)
...
def FST_norm_3(xs, n_samples, cut):
    len_in_cut = np.count_nonzero(cut == True)
    len_out_cut = np.count_nonzero(cut == False)
    len_xs = len_in_cut + len_out_cut
    p_in = (1 / (2 * len_in_cut)) * np.sum(xs * cut[:, np.newaxis], axis=0)
    ...
    #norm = np.minimum(len_in_cut, len_out_cut)/np.maximum(len_in_cut, len_out_cut)
    #norm = 1 / (1 + math.exp(-100 * (len_in_cut/len_xs)*(1-(len_out_cut/len_xs) -
    #                                0.01)))
    #norm = abs(len_in_cut / len_xs - len_out_cut / len_xs)
    #print(norm)
    FST_exp = ((np.sum(F_in) / xs.shape[1]) + (np.sum(F_out) / xs.shape[1]))

    if np.minimum(len_in_cut, len_out_cut) < 0.01*len_xs:
        return 2* (1/FST_exp)
    return (1/FST_exp)

def FST_norm_4(xs, n_samples, cut):
    len_in_cut = np.count_nonzero(cut == True)
    len_out_cut = np.count_nonzero(cut == False)
    len_xs = len_in_cut + len_out_cut
    p_in = (1 / (2 * len_in_cut)) * np.sum(xs * cut[:, np.newaxis], axis=0)
    ...
    F_out = np.abs(1 - ((p_out * (1 - p_out)) / (p * (1 - p))))
    #FST_exp = 0.5 * ((np.sum(F_in) / xs.shape[1]) + (np.sum(F_out) / xs.shape[1]))
    norm = math.sqrt(np.power((len_in_cut/len_xs),0.5)*np.power((len_out_cut/len_xs),
                                                                0.5))
    print("norm:", norm)
    #norm = 1/np.minimum(len_in_cut, len_out_cut)
    FST_exp = ((np.sum(F_in) / xs.shape[1]) + (np.sum(F_out) / xs.shape[1]))
    return (1/FST_exp)/norm
...
def FST_wikipedia_fast(xs, n_samples, cut):
    ...
    n_in = np.count_nonzero(cut == True)
    n_out = np.count_nonzero(cut == False)
    n = n_in + n_out
    # print("True 2:", len_in_cut)
    # print("False 2:", len_out_cut)

    p_in = (1 / (2 * n_in)) * np.sum(xs * cut[:, np.newaxis], axis=0)
    p_out = (1 / (2 * n_out)) * np.sum(xs * (~cut)[:, np.newaxis], axis=0)
    p = ((n_in / n) * p_in) + ((n_out / n) * p_out)
    #print("p_in:", p_in)
    #print("p_out:", p_out)
    #print("p:", p)

    FST_wiki = (p * (1 - p) - (n_in/n)*(p_in * (1 - p_in)) - (
        n_out/n)*(
            p_out
                                                                                * (
            1-p_out)))/(p * (1 - p))
    ...
    FST_exp = np.sum(FST_wiki) / xs.shape[1]
    #print("FST_exp:", FST_exp)
    #print("FST exp:", FST_exp)
    return 1 / FST_exp
