...
from src.tree_tangles import ContractedTangleTree, tangle_computation, compute_soft_predictions_children#, mut_props_per_terminal_node, get_terminal_node_properties
...
def tangles_in_pop_gen(sim_data, n, rho, theta, agreement, seed, noise_param, output_directory = '', plot=True):
    ...
    soft_prediction = compute_soft_predictions_children(node=contracted_tree.root,
                                  cuts=bipartitions,
                                  weight=weight,
                                  verbose=3)#

    #print("soft prediction:", soft_prediction)
    ...
    if plot:
        ...
        ## plot the tree
        filename1 = "tree_n_"+str(n)+"_rho_"+str(rho)+"_theta_"+str(theta)+"_seed_"+str(seed)+"_agreement_"+str(agreement)+"_noise_"+str(noise)+".svg"
        filename2 = "contracted_n_"+str(n)+"_rho_"+str(rho)+"_theta_"+str(theta)+"_seed_"+str(seed)+"_agreement_"+str(agreement)+"_noise_"+str(noise)+".svg"
